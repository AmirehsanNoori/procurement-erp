...
// Body (all optional):
//   tenants: [{ name, code }]      default: one tenant { name:'سازمان اصلی', code:'ORG' }
//   admin:   { email, password, fullName }   default: from ADMIN_* env
//   plan:    'free' | 'pro' | 'enterprise'   default: 'enterprise' (unlimited)
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const body = (req.body ?? {}) as {
      tenants?: { name: string; code: string }[];
      admin?: { email?: string; password?: string; fullName?: string };
      plan?: string;
    };

    const tenantDefs = body.tenants?.length
      ? body.tenants
      : [{ name: 'سازمان اصلی', code: 'ORG' }];
    const plan = body.plan ?? 'enterprise';

    // 1) Tenants — created with the requested plan (enterprise => unlimited users)
    const tenants = [];
    for (const t of tenantDefs) {
      tenants.push(
        await prisma.tenant.upsert({
          where: { code: t.code },
          create: { name: t.name, code: t.code, plan, maxUsers: plan === 'enterprise' ? 9999 : 25 },
          update: { name: t.name, plan, maxUsers: plan === 'enterprise' ? 9999 : 25 },
        })
      );
    }
    log.push(`tenants: ${tenants.map((t) => `${t.code} (${t.plan})`).join(', ')}`);
    ...
    // 4) Admin user (super admin, manager in all created tenants)
    const adminEmail = (body.admin?.email ?? process.env.ADMIN_EMAIL ?? 'admin@procurement.local').toLowerCase();
    const adminPassword = body.admin?.password ?? process.env.ADMIN_PASSWORD ?? 'Admin@12345';
    const adminFullName = body.admin?.fullName ?? process.env.ADMIN_FULL_NAME ?? 'System Administrator';
    const bcryptRounds = Number(process.env.BCRYPT_ROUNDS ?? 10);

    const passwordHash = await bcrypt.hash(adminPassword, bcryptRounds);
    const admin = await prisma.user.upsert({
      where: { email: adminEmail },
      create: { fullName: adminFullName, email: adminEmail, passwordHash, isSuperAdmin: true, isActive: true },
    ...
    log.push(`admin: ${admin.email} (super admin)`);
